...
- (id)configBody:(id)body {
    ...
    if ([body isKindOfClass:[NSDictionary class]]) {
        return [[self filterDataType:((NSDictionary *)body).mutableCopy] mj_JSONData];
    }
...
- (NSDictionary *)filterDataType:(NSMutableDictionary *)params {
    for (NSString *key in params.allKeys) {
        id value = params[key];
        if ([value isKindOfClass:[NSData class]]) {
            params[key] = [(NSData *) value base64EncodedStringWithOptions:1];
            continue;
        }
        if ([value isKindOfClass:[NSDictionary class]]) {
            params[key] = [self filterDataType:[(NSDictionary *) value mutableCopy]];
        }
    }
    return params;
}
